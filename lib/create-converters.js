...
function htmlLoop(i, apps, callback) {
	if(i === apps.length) { callback() }
		...
		apps[i].link = link
		fs.writeFile('public/' + apps[i].folder + '/index.html', html(apps[i]), function(err) {
...
function jsLoop(i, apps, callback) {
	if(i === apps.length) { callback() }
		...
		var cmd1 = 'minify --output public/' + apps[i].folder + '/script.min.js apps/' + apps[i].folder + '/public/script.js'
		run(cmd0, function() {
			run(cmd1, function() {
