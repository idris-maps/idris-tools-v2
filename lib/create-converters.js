var apps = require('../data/converters.json')
var html = require('./html-templates/converter')
var exec = require('child_process').exec
var fs = require('fs')
var link = {
	back: 'http://www.idris-maps.com',
	css: '../css/converter.css'
}
...
function htmlLoop(i, apps, callback) {
	if(i === apps.length) { callback() }
		...
		apps[i].link = link
		fs.writeFile('public/convert/' + apps[i].folder + '/index.html', html(apps[i]), function(err) {
...
function jsLoop(i, apps, callback) {
	if(i === apps.length) { callback() }
		...
		var cmd1 = 'minify --output public/convert/' + apps[i].folder + '/script.min.js apps/' + apps[i].folder + '/public/script.js'
		run(cmd0, function() {
			run(cmd1, function() {
